...
# Handle package name modification based on build type (mutually exclusive)
if [ "$NON_CUDA_BUILD" = "1" ]; then
...
elif [ "$ASCEND_BUILD" = "1" ]; then
    echo "Modifying package name for Ascend build"
    # Backup original pyproject.toml
    cp pyproject.toml pyproject.toml.backup
    # Replace package name and description
    sed -i 's/name = "mooncake-transfer-engine"/name = "mooncake-transfer-engine-ascend"/' pyproject.toml
    ...
    echo "Package name modified to: mooncake-transfer-engine-ascend"
